...
class WitsV3System:
    ...
    async def initialize(self):
        ...
        try:            # Initialize LLM interface
            self.llm_interface = OllamaInterface(
                config=self.config
            )            
            # Test LLM connection (simple test)
            logger.info("LLM interface initialized")  # Fixed unicode logging
            ...
            if self.config.memory_manager.backend == "basic":
                ...
                logger.info("Memory manager initialized with basic backend")  # Fixed unicode logging
            else:
            ...
            logger.info(f"Tool registry initialized with {len(self.tool_registry.tools)} tools")  # Fixed unicode logging
            ...
            logger.info("LLM-driven orchestrator initialized")  # Fixed unicode logging
            ...
            logger.info("WITS Control Center initialized")  # Fixed unicode logging
            
            logger.info("WitsV3 system fully initialized and ready!")  # Fixed unicode logging (removed emoji)
...
async def run_cli():
    """Run WitsV3 in CLI mode."""    print("Starting WitsV3 - CLI Mode")
    print("=" * 50)
...
def print_help():
    """Print help information."""    help_text = """
WitsV3 CLI Commands:
