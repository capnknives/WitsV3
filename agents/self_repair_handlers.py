...
async def handle_health_check(agent, session_id: str) -> AsyncGenerator[StreamData, None]:
    """
    ...
    yield agent.stream_observation(
        f"CPU: {current_metrics.cpu_usage:.1f}%, Memory: {current_metrics.memory_usage:.1f}%"
    )
    yield agent.stream_observation(
        f"Disk: {current_metrics.disk_usage:.1f}%, Response Time: {current_metrics.response_time:.2f}s"
    )
...
async def handle_issue_diagnosis(agent, session_id: str) -> AsyncGenerator[StreamData, None]:
    """
...
async def handle_system_repair(
    agent, task_analysis: dict[str, Any], session_id: str
    ...
    for issue_id, issue in list(
        agent.detected_issues.items()
    ...
    yield agent.stream_result(
        f"Repair summary: {repairs_successful}/{repairs_attempted} fixes successful"
    )
...
async def handle_performance_optimization(
    ...
    Recent metrics:
    - Average CPU: {sum(m.cpu_usage for m in agent.system_metrics[-10:]) / 10}%
    - Average Memory: {sum(m.memory_usage for m in agent.system_metrics[-10:]) / 10}%
    - Average Response Time: {sum(m.response_time for m in agent.system_metrics[-10:]) / 10}s
    - Error Rate: {sum(m.error_rate for m in agent.system_metrics[-10:]) / 10 * 100}%
    ...
    await agent.store_memory(
        ...
        importance=0.8,
        metadata={"optimizations_applied": len(auto_optimizations), "session_id": session_id},
    )


async def handle_capability_evolution(agent, session_id: str) -> AsyncGenerator[StreamData, None]:
    """
...
async def handle_failure_learning(agent, session_id: str) -> AsyncGenerator[StreamData, None]:
    """
...
async def handle_tool_monitoring(agent, session_id: str) -> AsyncGenerator[StreamData, None]:
    """
    ...
    for tool_name, failure_count in failing_tools.items():
        ...
        issue = await create_issue_record(
            issue_desc, "tool_failure", "high" if failure_count > 5 else "medium"
        )
        agent.detected_issues[issue.id] = issue
    ...
    yield agent.stream_result(
        f"Tool repair summary: {repairs_successful}/{repairs_attempted} repairs successful"
    )
...
async def handle_general_maintenance(
    ...
    yield agent.stream_result(
        f"Maintenance completed: {len(completed_tasks)}/{len(maintenance_tasks)} tasks successful"
    )
    ...
    await agent.store_memory(
        ...
        importance=0.6,
        metadata={"tasks_completed": completed_tasks, "session_id": session_id},
    )
