...
from agents.book_writing_agent import BookWritingAgent, Chapter
from core.content_fallback_system import ContentAwareAgent
...
class EnhancedBookWritingAgent(BookWritingAgent, ContentAwareAgent):
    ...
    async def _handle_chapter_writing(
        ...
        content_description = (
            f"{task_analysis['topic']} {task_analysis['genre']} {task_analysis.get('style', '')}"
        )
        ...
        if content_categories:
            yield self.stream_thinking(
                f"Detected content categories: {', '.join(content_categories)}"
            )
        ...
        try:
            ...
            if content_categories:
                yield self.stream_observation(
                    f"Content categories: {', '.join(content_categories)}"
                )
...
async def example_enhanced_writing():
    ...
    book_agent = EnhancedBookWritingAgent(
        "EnhancedBookWriter", config, llm_interface, memory_manager
    )
    ...
    async for stream in book_agent.handle_content_request_with_context(
        "Write a heartwarming story about friendship", content_type="general"
    ):
    ...
    async def interactive_writing(self):
        ...
        while True:
            ...
            async for stream in self.book_agent.handle_content_request_with_context(
                user_input, user_preferences=user_prefs
            ):
