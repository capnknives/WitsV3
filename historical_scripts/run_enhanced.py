...
class WitsV3System:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if HAS_FILE_WATCHER and self.config.auto_restart_on_file_change and not IS_TEST_MODE:
    ...
    async def test_system(self) -> bool:
        """
        Test system functionality without starting CLI.
        Returns True if all tests pass.
        """
        logger.info("Running system validation tests...")
        
        try:
            # Test 1: Basic component check
            if not all([self.llm_interface, self.memory_manager, self.tool_registry, self.control_center]):
            ...
            # Test 3: Process a simple test message
            test_message = "What is 2+2?"
            try:
                response = await self.process_user_input(test_message)
                if not response:
                    logger.error("Empty response from test message")
                    return False
...
async def run_test_mode():
    """Run WitsV3 in test/validation mode."""
    print("🧪 WitsV3 Test Mode - Validating System")
    print("=" * 50)
    ...
    try:
        ...
        # Run tests
        test_passed = await wits_system.test_system()
        
        if test_passed:
            print("✅ WitsV3 system validation PASSED")
            print("🎉 All components are working correctly")
            return 0
        else:
            print("❌ WitsV3 system validation FAILED")
