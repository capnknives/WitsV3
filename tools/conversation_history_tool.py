...
class ReadConversationHistoryTool(BaseTool):
    ...
    async def execute(
        ...
        try:
            ...
            # Get recent messages
            recent_messages = (
                conversation_history.messages[-max_messages:]
                if max_messages > 0
    ...
    async def execute(
        ...
        try:
            ...
            assistant_messages = sum(
                1 for msg in conversation_history.messages if msg.role == "assistant"
            )
            ...
            if analysis_type == "summary":
                result = {
                    ...
                    "last_speaker": (
                        conversation_history.messages[-1].role if message_count > 0 else None
                    ),
            ...
            self.logger.info(
                f"Analyzed conversation: {analysis_type} analysis on {message_count} messages"
            )
            return result
...
async def test_conversation_tools():
    ...
    conversation.add_message(
        "assistant", "Great choice! Here's a short sci-fi story about time travel..."
    )
