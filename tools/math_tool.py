...
class MathTool(BaseTool):
    ...
    async def execute(
        self, operation: str, data: list[float] | dict[str, Any], **kwargs: Any
    ) -> dict[str, Any]:
        """
    ...
    async def _regression_analysis(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if len(x) != len(y):
                return {"success": False, "error": "x and y must have the same length"}
    ...
    async def _probability_calculations(
        ...
        try:
            if "distribution" not in data:
                return {"success": False, "error": "Missing distribution type"}
            ...
            if dist_type == "normal":
                ...
                if x is None:
                    return {"success": False, "error": "Missing x value for normal distribution"}

                # Validate parameters
                if std <= 0:
                    return {"success": False, "error": "Standard deviation must be positive"}
                ...
                return {"success": True, "results": {"pdf": float(pdf), "cdf": float(cdf)}}
                ...
                # Validate parameters
                if not (0 <= p <= 1):
                    return {"success": False, "error": "Probability p must be between 0 and 1"}
                ...
                return {"success": True, "results": {"pmf": float(pmf), "cdf": float(cdf)}}

            else:
                return {"success": False, "error": f"Unsupported distribution: {dist_type}"}

        except Exception as e:
            return {"success": False, "error": str(e)}

    async def _matrix_operations(self, data: dict[str, Any], **kwargs: Any) -> dict[str, Any]:
        """Perform matrix operations."""
        try:
            if "operation" not in data or "matrices" not in data:
                return {"success": False, "error": "Missing operation or matrices"}

            operation = data["operation"]
            matrices = data["matrices"]

            if not isinstance(matrices, list) or len(matrices) < 1:
                return {"success": False, "error": "Invalid matrices data"}
    ...
    async def _optimization(self, data: dict[str, Any], **kwargs: Any) -> dict[str, Any]:
        """Perform optimization calculations."""
        try:
            if "function" not in data or "method" not in data:
                return {"success": False, "error": "Missing function or method"}
            ...
            if method == "minimize":
                ...
                if bounds:
                    result = stats.minimize(
                        func, x0=initial_guess if initial_guess else None, bounds=bounds
                    )
                else:
                    result = stats.minimize(func, x0=initial_guess)
        ...
        return {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "data": {"type": ["array", "object"], "description": "The data to operate on"},
                },
