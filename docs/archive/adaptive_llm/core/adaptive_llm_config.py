...
class DynamicModuleSettings:
    ...
    quantization_thresholds: dict[str, int] = field(
        default_factory=lambda: {
...
class SemanticCacheSettings:
    ...
    cache_file_path: str = field(
        default_factory=lambda: os.path.join("cache", "semantic_cache.json")
    )
...
class AdaptiveLLMSettings:
    ...
    # Component settings
    complexity_analyzer_settings: ComplexityAnalyzerSettings = field(
        default_factory=ComplexityAnalyzerSettings
    )
