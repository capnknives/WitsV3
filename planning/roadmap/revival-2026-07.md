...
1. Optionally add `ANTHROPIC_API_KEY` if the ask-Claude escalation should work — now documented in `.env.example` (July 7 2026), still requires Richard to supply the actual key. Brave key: DONE (`BRAVE_SEARCH_API_KEY` added; `web_search` merges Tavily+Brave). Supabase token: N/A, no project exists anymore.
