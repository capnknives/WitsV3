...
from core.llm_interface import BaseLLMInterface, OllamaInterface, get_llm_interface
...
async def test_prepare_payload_override_params(mock_config: WitsV3Config):
    ...
    expected = {
        ...
        "prompt": "Test prompt",
        "stream": True,
        "options": {"temperature": 0.8, "num_predict": 100, "stop": ["stop1", "stop2"]},
    }
    assert payload == expected
...
async def test_generate_text_success(mock_config: WitsV3Config, mock_async_client: MagicMock):
    ...
    mock_async_client.post.assert_called_once_with(
        f"{mock_config.ollama_settings.url}/api/generate", json=expected_payload
    )
...
@pytest.mark.asyncio
async def test_generate_text_http_status_error(
    mock_config: WitsV3Config, mock_async_client: MagicMock
):
    ...
    error = httpx.HTTPStatusError(
        "Internal Server Error", request=MagicMock(), response=mock_httpx_response
    )
...
@pytest.mark.asyncio
async def test_stream_text_http_status_error(
    mock_config: WitsV3Config, mock_async_client: MagicMock
):
...
@pytest.mark.asyncio
async def test_stream_text_json_decode_error(
    mock_config: WitsV3Config, mock_async_client: MagicMock, caplog
):
...
async def test_get_embedding_success(mock_config: WitsV3Config, mock_async_client: MagicMock):
    ...
    expected_payload = {"model": "test-embedding-model", "prompt": "Embed this text"}
    mock_async_client.post.assert_called_once_with(
        f"{mock_config.ollama_settings.url}/api/embeddings", json=expected_payload
    )
    mock_httpx_response.raise_for_status.assert_called_once()
...
@pytest.mark.asyncio
async def test_get_embedding_override_model(
    mock_config: WitsV3Config, mock_async_client: MagicMock
):
    ...
    expected_payload = {"model": "override-embed-model", "prompt": "Embed this"}  # Overridden model
    mock_async_client.post.assert_called_once_with(
        f"{mock_config.ollama_settings.url}/api/embeddings", json=expected_payload
    )
...
@pytest.mark.asyncio
async def test_get_embedding_http_status_error(
    mock_config: WitsV3Config, mock_async_client: MagicMock
):
    ...
    error = httpx.HTTPStatusError(
        "Service Unavailable", request=MagicMock(), response=mock_httpx_response
    )
    mock_httpx_response.raise_for_status = MagicMock(side_effect=error)
    mock_async_client.post.return_value = mock_httpx_response
    ...
    assert mock_async_client.post.call_count == mock_config.ollama_settings.retry_attempts
    assert (
        mock_httpx_response.raise_for_status.call_count
        == mock_config.ollama_settings.retry_attempts
    )
