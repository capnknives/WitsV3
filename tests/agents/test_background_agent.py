...
async def test_task_execution(background_agent):
    ...
    background_agent.tasks_config = {"tasks": {"test_task": {"enabled": True, "settings": {}}}}
    ...
    background_agent._execute_task.assert_called_once_with(
        "test_task", {"enabled": True, "settings": {}}
    )
...
async def test_memory_maintenance(background_agent, memory_manager):
    ...
    low_importance_segment = Mock(
        id="low-imp-seg", timestamp=datetime.now() - timedelta(days=1), importance=0.1
    )
