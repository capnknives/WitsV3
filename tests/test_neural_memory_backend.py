...
async def test_neural_memory_backend():
    ...
    try:
        ...
        print(
            "✅ Neural memory backend initialized successfully"
        )  # Test 1: Add a few memory segments
        test_segments = [
            MemorySegment(
        ...
        if hasattr(backend, "neural_web"):
            ...
            for (source_id, target_id), connection in backend.neural_web.connections.items():
                ...
                if connection.source_id is None or connection.source_id == "None":
                    raise ValueError(
                        f"Found connection with None source_id: {connection.source_id}"
                    )
                if connection.target_id is None or connection.target_id == "None":
                    raise ValueError(
                        f"Found connection with None target_id: {connection.target_id}"
                    )
