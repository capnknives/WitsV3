...
class MetricsManager:
    def __init__(self):
        ...
        self.metrics: dict[str, list[dict[str, Any]]] = {"system": [], "llm": [], "errors": []}
    ...
    def record_metric(self, metric_type: str, data: dict[str, Any]) -> None:
        ...
        self.metrics[metric_type].append({**data, "timestamp": datetime.now().isoformat()})
        self.logger.debug(f"Recorded {metric_type} metric: {data}")

    def record_error(self, error_type: str) -> None:
        """Record an error with timestamp."""
        self.metrics["errors"].append({"type": error_type, "timestamp": datetime.now().isoformat()})
