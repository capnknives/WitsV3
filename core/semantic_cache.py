...
class SemanticCache:
    ...
    def __init__(self, config: WitsV3Config, llm_interface: BaseLLMInterface):
        ...
        self.embeddings = np.zeros(
            (self.settings.cache_size, self.settings.embedding_dim), dtype=np.float16
        )
        self.metadata = []
        self.current_idx = 0
        self.is_full = False
    ...
    async def add_pattern(
        ...
        try:
            ...
            if (
                self.settings.enable_persistence
                and len(self.metadata) % self.settings.persistence_interval == 0
            ):
    ...
    async def find_similar(
        self, input_text: str, threshold: float | None = None, top_k: int = 1
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            norms = np.linalg.norm(self.embeddings[:num_patterns], axis=1) * np.linalg.norm(
                input_embedding_np
            )
            ...
            for idx in top_indices:
                if similarities[idx] > effective_threshold:
                    ...
                    self.logger.debug(
                        f"Found similar pattern with similarity: {similarities[idx]:.4f}"
                    )
                    return result
    ...
    def _update_user_patterns(self, metadata: dict[str, Any]) -> None:
        ...
        if len(self.user_patterns["complexity_history"]) > self.settings.max_user_patterns:
            self.user_patterns["complexity_history"] = self.user_patterns["complexity_history"][
                -self.settings.max_user_patterns :
            ]
        ...
        perf["count"] += 1
        perf["avg_time"] = (
            perf["avg_time"] * (perf["count"] - 1) + metadata.get("generation_time", 0.0)
        ) / perf["count"]
        perf["avg_quality"] = (
            perf["avg_quality"] * (perf["count"] - 1) + metadata.get("quality", 0.0)
        ) / perf["count"]
    ...
    async def _load_cache(self) -> None:
        ...
        try:
            ...
            with open(self.settings.cache_file_path) as f:
            ...
            self.user_patterns = load_data.get(
                "user_patterns",
                {
                    "common_domains": {},
                    "complexity_history": [],
            ...
            self.logger.info(
                f"Cache loaded from {self.settings.cache_file_path} with {len(self.metadata)} patterns"
            )
