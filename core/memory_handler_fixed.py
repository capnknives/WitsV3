...
# Import the integration module for compatibility
from core.synthetic_brain_integration import (
    get_compatible_memory_manager,
    get_compatible_working_memory,
    get_compatible_knowledge_graph,
    export_memory_safely,
    summarize_memory_safely
)
...
class MemoryHandler:
    ...
    def __init__(self, config_path: str = "config/wits_core.yaml"):
        ...
        # Initialize with compatible implementations
        self.memory_manager = get_compatible_memory_manager(self.config)
        self.working_memory = get_compatible_working_memory(self.config)
        self.knowledge_graph = get_compatible_knowledge_graph(self.config)
    ...
    async def _store_episodic_memory(self, segment: MemorySegment) -> None:
        ...
        try:
            # Use memory manager to store in vector database
    ...
    async def _store_semantic_memory(self, segment: MemorySegment) -> None:
        ...
        try:
            # Store in the memory manager
            key = f"semantic:{segment.id}"
            await self.memory_manager.store(key, segment.content, segment.metadata)

            # Extract concepts and add to knowledge graph if available
            concepts = segment.metadata.get("concepts", [])
            if concepts and hasattr(self.knowledge_graph, "add_concept"):
    ...
    async def _recall_episodic(self, query: str, limit: int, threshold: float) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error recalling episodic memories: {e}")
            return []
    ...
    async def _recall_semantic(self, query: str, limit: int, threshold: float) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Error searching memory manager: {mem_err}")

            # Also check knowledge graph for relevant concepts if available
            try:
                if hasattr(self.knowledge_graph, "search_concepts"):
                    concepts = self.knowledge_graph.search_concepts(query, limit=limit)
                    for concept in concepts:
                        try:
                            concept_id = getattr(concept, "id", f"concept:{uuid.uuid4()}")
                            concept_desc = getattr(concept, "description", "No description")
                            concept_rel = getattr(concept, "relevance", 0.8)
                            ...
                            # Handle different concept object structures
                            self.logger.warning("Could not extract concept attributes")
            except Exception as kg_err:
                self.logger.warning(f"Error searching knowledge graph: {kg_err}")

            # Sort by relevance and limit results
            filtered_results.sort(key=lambda x: x.get("relevance", 0), reverse=True)
            return filtered_results[:limit]
        except Exception as e:
            self.logger.error(f"Error recalling semantic memories: {e}")
            return []
    ...
    async def get_current_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.knowledge_graph, "get_active_concepts"):
                self.context.active_concepts = set(self.knowledge_graph.get_active_concepts())
        except Exception as e:
            self.logger.warning(f"Error updating memory context: {e}")
    ...
    async def consolidate_memories(self) -> None:
        ...
        try:
            summary = await summarize_memory_safely(summary_text)
    ...
    async def export_all_memories(self, export_path: str) -> None:
        ...
        """
        await export_memory_safely(export_path)
