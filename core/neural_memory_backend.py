...
class NeuralMemoryBackend(BaseMemoryBackend):
    ...
    def __init__(self, config: WitsV3Config, llm_interface: BaseLLMInterface):
        ...
        self.connection_strength_threshold = getattr(
            neural_settings, "connection_strength_threshold", 0.7
        )
    ...
    async def prune_memory(self):
        ...
        try:
            ...
            if len(self.segments) <= max_segments:
                logger.debug(
                    f"No pruning needed. Current segments: {len(self.segments)}, max: {max_segments}"
                )
                return
            ...
            logger.info(
                f"Pruning {num_to_prune} segments (current: {len(self.segments)}, max: {max_segments})"
            )
            ...
            for segment in segments_to_remove:
                try:
                    if (
                        hasattr(self.neural_web, "concepts")
                        and segment.id in self.neural_web.concepts
                    ):
            ...
            logger.info(
                f"Pruning complete. Removed {num_to_prune} segments, kept {len(self.segments)} segments"
            )
    ...
    async def _create_neural_concept(self, segment_id: str, segment: MemorySegment):
        ...
        if not self._validate_segment_id(segment_id):
            logger.error(
                f"VALIDATION FAILED - Invalid segment_id: '{segment_id}', cannot create neural concept"
            )
            return

        if not segment:
            logger.error(
                f"VALIDATION FAILED - Invalid segment provided for segment_id: {segment_id}"
            )
            return
        ...
        try:
            ...
            if hasattr(self.neural_web, "add_concept"):
                ...
                logger.warning("Neural web does not support add_concept method")

        except Exception as e:
            logger.error(
                f"EXCEPTION - Failed to create neural concept for segment {segment_id}: {e}"
            )
    ...
    async def _auto_connect_concepts(self, new_concept_id: str, segment: MemorySegment):
        ...
        if not self._validate_segment_id(new_concept_id):
            logger.error(
                f"VALIDATION FAILED - Invalid new_concept_id: '{new_concept_id}', cannot auto-connect"
            )
            return

        if not segment:
            logger.error(
                f"VALIDATION FAILED - Invalid segment provided for concept_id: {new_concept_id}"
            )
            return
        ...
        try:
            ...
            self.neural_web.concepts[new_concept_id]
                        ...
                        if content1_words and content2_words:
                            ...
                            if similarity >= self.connection_strength_threshold:
                                ...
                                if hasattr(self.neural_web, "connect_concepts"):
                                    ...
                                    connections_made += 1
                                    logger.debug(
                                        f"Connected {new_concept_id} to {concept_id} (similarity: {similarity:.3f})"
                                    )
            ...
            logger.debug(
                f"SUCCESS - Auto-connected concept {new_concept_id} to {connections_made} other concepts"
            )
    ...
    def _determine_concept_type(self, segment: MemorySegment) -> str:
        ...
        try:
            ...
            if any(
                ...
                word in content_lower
                for word in ["task", "todo", "action", "do", "complete", "finish"]
            ):
                return "task"
            elif any(
                word in content_lower for word in ["fact", "is", "are", "definition", "means"]
            ):
                return "fact"
    ...
    def _determine_relationship_type(self, content1: str, content2: str) -> str:
        ...
        try:
            ...
            if any(word in c1_lower for word in causal_words) or any(
                word in c2_lower for word in causal_words
            ):
                return "causal"

            # Check for temporal relationships
            temporal_words = ["before", "after", "then", "next", "previously", "following"]
            if any(word in c1_lower for word in temporal_words) or any(
                word in c2_lower for word in temporal_words
            ):
                return "temporal"

            # Check for similarity
            similarity_words = ["similar", "like", "same", "resembles", "comparable"]
            if any(word in c1_lower for word in similarity_words) or any(
                word in c2_lower for word in similarity_words
            ):
                return "similarity"

            # Check for contrast
            contrast_words = ["different", "unlike", "however", "but", "opposite"]
            if any(word in c1_lower for word in contrast_words) or any(
                word in c2_lower for word in contrast_words
            ):
                return "contrast"
    ...
    async def _load_neural_web(self):
        ...
        try:
            if self.neural_web_path.exists():
                with open(self.neural_web_path, encoding="utf-8") as f:
                    neural_data = json.load(f)

                # Custom deserialization - restore concepts first
                for _concept_id, concept_data in neural_data.get("concepts", {}).items():
                    # Skip concepts with invalid IDs
                    stored_concept_id = concept_data.get("id")
                    if (
                        stored_concept_id is None
                        or stored_concept_id == "null"
                        or stored_concept_id == ""
                    ):
                        import uuid

                        valid_concept_id = f"concept_{uuid.uuid4().hex[:8]}"
                        logger.warning(
                            f"Found concept with invalid ID ({stored_concept_id}), assigning new ID: {valid_concept_id}"
                        )
                    else:
                ...
                # Restore connections
                for _conn_key, conn_data in neural_data.get("connections", {}).items():
                    ...
                    if (
                        source_id in self.neural_web.concepts
                        and target_id in self.neural_web.concepts
                    ):
                        await self.neural_web.connect_concepts(
                            source_id=source_id,
                            target_id=target_id,
                        ...
                        logger.warning(
                            f"Skipping connection {source_id} -> {target_id}: one or both concepts don't exist"
                        )

                logger.info(
                    f"Loaded neural web with {len(self.neural_web.concepts)} concepts and {len(self.neural_web.connections)} connections"
                )
            else:
