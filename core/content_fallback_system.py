...
class ContentFallbackManager:
    ...
    async def generate_with_fallback(
        ...
        try:
            ...
            response = await self.llm_interface.generate_text(
                prompt=prompt, model=primary_model, temperature=temperature, max_tokens=max_tokens
            )
            ...
            return await self._attempt_fallback(
                prompt, temperature, max_tokens, error_fallback=True, original_error=str(e)
            )
    ...
    async def generate_streaming_with_fallback(
        ...
        try:
            async for chunk in self.llm_interface.stream_text(
                prompt=prompt, model=primary_model, temperature=temperature
            ):
    ...
    def _is_content_refused(self, response: str) -> bool:
        ...
        if len(response) < 100 and any(
            word in response_lower for word in ["cannot", "unable", "sorry", "apologize"]
        ):
            return True

        # Responses that immediately pivot to alternatives
        if (
            response_lower.startswith(("instead", "however", "alternatively"))
    ...
    async def _attempt_fallback(
        ...
        temperature: float,
        max_tokens: int | None,
        refusal: ContentRefusal | None = None,
    ...
    async def _stream_uncensored(
        ...
        try:
            async for chunk in self.llm_interface.stream_text(
                prompt=prompt, model=uncensored_model, temperature=temperature
            ):
                yield {
                    "type": "content",
                    "content": chunk,
    ...
    def _modify_prompt_for_uncensored(
        self, prompt: str, refusal: ContentRefusal | None = None
    ) -> str:
        """Modify prompt to work better with uncensored models"""

        # Add context that this is for creative/fictional purposes
        if any(
            word in prompt.lower() for word in ["story", "character", "fiction", "novel", "book"]
        ):
    ...
    def should_preempt_with_uncensored(
        self, prompt: str, content_category: str | None = None
    ) -> bool:
        """
        ...
        for past_refusal in self.refusal_history[-10:]:  # Check last 10 refusals
            if any(
                word in prompt_lower for word in past_refusal.original_prompt.lower().split()[:5]
            ):
                return True
...
class ContentAwareAgent:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.content_fallback = ContentFallbackManager(self.llm_interface, self.config)
