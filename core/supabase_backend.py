...
class SupabaseMemoryBackend(BaseMemoryBackend):
    ...
    def __init__(
        ...
        self.supabase: Client = create_client(config.supabase.url, config.supabase.key)
        ...
        try:
            ...
            data = {
                ...
                "embedding": (
                    ...
                    if hasattr(segment.embedding, "tolist")
                    else segment.embedding
                ),
    ...
    async def get_recent_segments(
        ...
        try:
            ...
            def do_query():
                query = (
                    self.supabase.table("memory_segments")
                    .select("*")
        ...
        try:
            ...
            if to_prune > 0:
                ...
                for data in result.data[:to_prune]:
                    segment_id = data["id"]
                    await asyncio.to_thread(
                        lambda sid=segment_id: self.supabase.table("memory_segments")
                        .delete()
                        .eq("id", sid)
                        .execute()
                    )
    ...
    async def _create_neural_concept(self, concept_id: str, segment: MemorySegment):
        ...
        try:
            await self.neural_web.create_concept(
                concept_id=concept_id, content=segment.content.text, metadata=segment.metadata
            )
        except Exception as e:
    ...
    def _combine_search_results(
        self, traditional: list[MemorySegment], neural: list[MemorySegment]
    ) -> list[MemorySegment]:
