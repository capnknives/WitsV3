...
WitsV3 is a local-first LLM orchestration system with:

- **Interfaces**: Web UI primary (`run_web.py`), CLI secondary (`run.py`)
- **Architecture**: ReAct orchestrator, tool registry, persistent memory, Ollama
- **Safety**: Verified code edits via `core/safe_code_editor.py`
- **Philosophy**: Async-first, streaming responses, config + `.env` for secrets

**Always check** [`planning/roadmap/suggested-features-2026-07.md`](planning/roadmap/suggested-features-2026-07.md) for priorities (root `TASK.md` is a redirect only). Product overview: [`README.md`](README.md).
...
4. **Update the forward roadmap** - `planning/roadmap/suggested-features-2026-07.md` (not root `TASK.md`)
...
7. **Use model reliability / routing** - Prefer `core/model_router.py` + reliability fallbacks
8. **Include error context** - Prefer `core/user_errors.py` for user-facing LLM outages
9. **Follow the July 2026 roadmap** - Neural web is optional/research, not the default focus
...
1. Read `planning/roadmap/suggested-features-2026-07.md` for priorities
2. Skim `README.md` if the change touches install or user-facing behavior
3. Check revival / clutter catalogs when touching old subsystems
4. Prefer patterns from shipped July 2026 work (web UI, verified edits, synthesis guard)
...
2. Update `planning/roadmap/suggested-features-2026-07.md` if status changed
...
See **`planning/roadmap/suggested-features-2026-07.md`**. July revival features (Web UI, Document RAG, search, MCP discovery, coding/self-repair, synthesis guard) are shipped. Default next work is polish: remaining test failures, clutter cleanup, CI lint expansion — **not** “Phase 2 Neural Web as primary product.”

When touching neural-web code: treat it as optional (`memory_manager.backend: neural`); confirm tools are actually registered before investing.
